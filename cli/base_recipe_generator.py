...
- """
    ...
    def generate_single_recipe(self, prompt: RecipePrompt) -> Dict[str, Any]:
        ...
        result = {
            ...
            "tokens_per_second": (lambda tps_list: sum(tps_list) / len(tps_list) if tps_list else 0)([r.get("tokens_per_second", 0) for r in mode_results.values() if r.get("tokens_per_second")]),
    ...
    def generate_all_recipes(self, checkpoint_path: str, parallel_generations: int = 2) -> Dict[str, Any]:
        ...
        comprehensive_results = {
            ...
            "avg_tokens_per_second": (lambda tps_list: sum(tps_list) / len(tps_list) if tps_list else 0)([r["tokens_per_second"] for r in results if r.get("tokens_per_second")]),
